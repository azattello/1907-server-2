...
const Filial = require('../models/Filial'); // Подключаем модель Filial
  ...
  try {
      ...
      const tracks = await Track.find(query) // Используем query для фильтрации
          ...
          .skip(startIndex);
           ...
           // Для каждого трека находим филиал по его ID (toFilial)
      const tracksWithFilial = await Promise.all(tracks.map(async (track) => {
        // Ищем филиал по ID
        let filialText = null;
        if (track.toFilial) {
            const filial = await Filial.findById(track.toFilial);
            if (filial) {
                filialText = filial.filialText; // Например, если в модели Filial есть поле name
            }
        }
        // Возвращаем трек с добавленным полем filialText
        return { ...track.toObject(), filialText };
      }));
