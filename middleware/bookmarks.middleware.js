      ...
      bookmarksPage.map(async (bookmark) => {
        if (!bookmark.trackId) {
          ...
          if (track) {
            ...
            const filial = await Filial.findById(track.toFilial); // Ищем филиал по ID
            const toFilial = filial ? filial.filialText : null;
            const toFilialAdress = filial ? filial.filialAddress : null;
            ...
            return {
              ...
              price: track.price,
              weight: track.weight,
              toFilial: toFilial, // Передаем текст филиала
              toFilialAdress: toFilialAdress
            };
          } else {
            // Трек не найден
            return {
